...
files2d = sorted(glob.glob('2D_SD/YZ_*'))
#print files2d
max_rho = np.zeros((2))
max_p = np.zeros((2))
max_u = np.zeros((2))
...
for indx, i in enumerate(files2d[0:20]):
	temp = i.split('YZ_')
...
#	plt.plot(f[:,0],f[:,2]/p, syms1[indx])
        plt.figure(1)
	plt.plot(f[:,0]-x_shock,f[:,4]/u,label='t=%d' % t )
        plt.figure(2)
	plt.plot(f[:,0]-x_shock,f[:,2]/p,label='t=%d' % t )
